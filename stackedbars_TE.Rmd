...
```{r}
myplot <- ggplot(datamelt, aes(fill=Tefam, x=variable, y=value)) + geom_bar(position="fill",stat="identity") + theme_classic() + xlab("") + ylab("Percentage of TEs") + coord_flip() + scale_fill_viridis(discrete=T,option="rocket")
myplot
```
...
hplot <- ggplot(data=master_data, aes(x=Age_MY,fill = genotype)) + geom_histogram(bins = 50) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                                                                              panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position = "none") + scale_fill_manual(values=c(rep(c("#F0E442","#117733"),5))) + scale_x_continuous(breaks=seq(0,8,by=.5),limits=c(0,8))  +xlab("Age in Million years")
